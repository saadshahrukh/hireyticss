...
export default function FreeTrialPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!fullName || !companyUrl || !gmail || !workEmail || !companyName) return;
    ...
    try {
      const response = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          formType: "14-Day Free Trial",
          subject: `[Free Trial Request] ${fullName} - ${companyName}`,
          data: {
            fullName,
            companyName,
            companyUrl,
            ...
            teamSize,
            trialPeriod: "14 Days",
            featuresIncluded: "15 AI Voice Minutes, Recall Engine, OCR Resume Parsing",
      ...
      setModalState({
        isOpen: true,
        status: "success",
        message: `We've accepted your 14-day free trial request! We will email your workspace login credentials to ${gmail} shortly.`,
      });
    } catch (err) {
      console.error(err);
      setModalState({
        isOpen: true,
        status: "error",
        message: "An error occurred while activating your trial. Please retry.",
      });
    } finally {
                ...
                <div className="relative z-10 space-y-6">
                  <div className="flex items-center gap-2.5">
                    <div className="flex h-10 w-10 items-center justify-center">
                      <Image src="/logo-icon-w.png" alt="Hireytics" width={35} height={35} className="object-contain" />
                    </div>
                ...
                <div className="relative z-10 mt-10 rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm">
                  ...
                  <div className="mt-3 flex items-center gap-2.5">
                    ...
                    <div className="text-xs">
                      <strong className="text-white">Mr Hasan</strong> • Head of People, Link Service Pvt.ltd
                    </div>
                  </div>
                </div>
              </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <div className="relative mt-1.5">
                          <BiLogoGmail className="absolute left-3 top-3 h-4 w-4 text-slate-400" />
                        <input  
                        type="text"
                        required
 ...
 <div>
                    ...
                    <div>
                      ...
                      <div className="relative mt-1.5">
                        <select
                          ...
                          className="w-full appearance-none rounded-xl border border-slate-200 bg-slate-50/70 px-3.5 py-2.5 text-xs sm:text-sm text-slate-800 outline-none transition focus:border-sky-500 focus:bg-white cursor-pointer"
                        >
                          {roleOptions.map((r) => (
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-sky-500 via-indigo-500 to-purple-600 py-3.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 transition-all hover:brightness-110 disabled:opacity-70 cursor-pointer"
                    >
                      {isSubmitting ? (
                        <>
      ...
      <StatusModal
        ...
        actionText={modalState.status === "success" ? "Return to Home" : "Retry"}
