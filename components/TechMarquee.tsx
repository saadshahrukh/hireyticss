...
export default function TechMarquee() {
  ...
  return (
    <section className="section-container max-w-6xl my-10 px-4 sm:px-6">
      <div className="relative w-full overflow-hidden rounded-3xl border border-slate-200/90 bg-white py-10 shadow-xs">
        ...
        {/* Fade masks */}
        <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-24 sm:w-32 bg-gradient-to-r from-white to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 sm:w-32 bg-gradient-to-l from-white to-transparent" />

        {/* Pure Logo Strip with B&W to Brand Color on Hover */}
        <div className="flex w-max animate-marquee items-center gap-14 sm:gap-20 hover:[animation-play-state:paused]">
          {marqueeList.map((logo, idx) => (
            <div
              key={`${logo.name}-${idx}`}
              className="group flex cursor-pointer items-center justify-center transition-transform duration-300 hover:scale-115"
              title={logo.name}
            >
